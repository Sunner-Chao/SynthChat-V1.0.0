...
impl ApiError {
    ...
    pub(crate) fn from_run(context: RequestContext, error: RunError) -> Self {
        match error {
            ...
            RunError::PersonaUnavailable => Self::new(
                context,
                StatusCode::UNPROCESSABLE_ENTITY,
                "Persona unavailable",
                "persona_unavailable",
                "The requested Persona does not exist in the Session Profile.",
                false,
            ),
            RunError::PersonaContextTooLarge => Self::new(
                context,
                StatusCode::UNPROCESSABLE_ENTITY,
                "Persona context too large",
                "persona_context_too_large",
                "The Persona and its enabled Worldbook sections exceed the bounded Run context limit.",
                false,
            ),
            RunError::ProductCatalogUnavailable => Self::new(
                context,
                StatusCode::SERVICE_UNAVAILABLE,
                "Product catalog unavailable",
                "product_catalog_unavailable",
                "The local Persona and Worldbook catalog could not be read safely.",
                true,
            ),
