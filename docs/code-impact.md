...
当前工程不是前后端分离项目，而是一个 React/Tauri 单体：
...
`src/panels/ToolPanels.tsx` 不能整文件删除。当前路由仍使用其中的 Memory、Worldbook、Plugins；但文件内旧 `McpPanel`、`AgentsPanel`、`SkillsPanel` 未被路由引用，应先拆分再删除死 UI。

## 5. UI 功能与 Hermes 能力映射
...
| Persona | SOUL/Profile 部分对应 | 一对一迁移或保留为前端展示元数据；不支持任意 Persona-Agent 组合 | 待产品决策 |
...
| Worldbook | 无直接等价 | 转为 Skill/上下文文件或下线 | 阻塞决策 |
| Moments | 无；当前 API 也是内存 mock | 下线或单独定义非 Agent 产品域 | 阻塞决策 |
| 微信账号绑定 | Hermes 有消息平台，但模型不同 | 作为阶段三之后的 messaging adapter，不应混入核心契约 | 阻塞决策 |
...
| Plugins 页面 | Hermes Dashboard plugins 不等于当前插件模型 | 重新定义或下线 | 阻塞决策 |
...
| `qrcode`、`lettre`、`imap`、`mailparse`、`native-tls`、`tokio-native-tls`、`rsa` | 随旧登录、邮件、微信/平台适配删除 |
...
本项目采用一条明确策略：阶段二先建立可运行的新骨架，然后在该阶段内一次性将旧 Agent runtime 移出主工程。尚未由新后端实现的界面显示明确的 unavailable 状态，不继续调用旧 runtime，也不以 mock 成功冒充能力。旧实现只存在于清洗后的 archive ref。
...
- `backend/` 与 `desktop/` 的 manifest 通过未使用依赖检查，无旧邮件、微信、ACP、自研加密或 Agent transport 依赖残留。
