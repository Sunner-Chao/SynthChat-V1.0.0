...
- Rust Toolset catalog 与 Profile 级启停管理使用共享 ProfileConfig revision/ETag，no-op 不改 revision，未知 catalog ID 不写入配置。当前 registry 注册 `session_search`、`skills_list`、`skill_view`、`read_file`、`search_files`、`write_file`、`patch`、`terminal`、`process`、`clarify`、`memory`、`web_search`、`web_extract` 和 `execute_code`：Provider 只能看到 Profile 已启用且 registry 已注册、先决条件已满足的严格 JSON schema。四个文件工具还要求 Run 绑定同 Profile 的可用 Workspace 并启用 `file` Toolset；Web 工具要求对应 Profile provider ready；`execute_code` 还要求可用的 host Python 3.8+。每次执行携带共享 `ToolExecutionControl`；RunService 将 terminal/process、Web 和 code execution 路由到异步 executor。`write_file`/`patch`、每次 `terminal`、model-driven `memory` 和整段 `execute_code` 脚本标记为 `ApprovalRequired`；`process` 在 prepare 后动态区分只读 `list/poll/log/wait` 和需新审批的 `kill/write/submit/close`。RunService 只能在 durable `once` 决策与 claim CAS 成功后构造授权。Skill 文件读取限制在对应 Profile 的技能目录，逐段拒绝 symlink/穿越并限制 UTF-8 内容大小。当前内部 schema 为 v13：v8 引入 owner-bound approval ledger，v9 引入绑定 Run/call/checkpoint/原始参数 SHA-256 的 immutable clarification ledger，v10 为 `tool_invocations` 增加 immutable `origin=provider|codeRpc`、`parent_call_id` 和单调 `rpc_sequence` 绑定，v11 增加持久 Run queue 与 epoch-fenced runtime lease，v12 增加 owner-bound async delivery record，v13 为 `sessions` 与 `session_versions` 增加受约束的 `persona_id`；tool plan、原始参数/结果、checkpoint、provider turn 和 process metadata 同样只保存在内部 journal，公开 Message/SSE 仅含有界脱敏投影。
...
Persona、Worldbook 与 Moments 是独立于 Agent runtime 的 Profile-scoped 产品域。Rust `ProductCatalogService` 将有界 JSON 记录持久化到 `HERMES_HOME/.synthchat/product-catalog-v1.db`，单项读取和 mutation 使用独立 strong product ETag。Session 或 `CreateRun` 可选择同 Profile Persona；准备首个模型 turn 时 RunService 冻结角色字段与所有启用且绑定的 Worldbook section。Persona 的模型参数按契约覆盖 Profile 默认值，显式 Run override 优先；`toolsEnabled` 与 `memoryEnabled` 分别控制该 Run 的工具和 Memory 注入。Moments 只提供显式 CRUD/comment/like，不主动触发模型。

微信扩展由 Rust `WechatService` 直接访问固定的 iLink HTTPS origin。Profile YAML 只保存非敏感设置与账号 metadata，扫码确认得到的 bot credential 只写 OS keychain；同 Profile Persona 最多绑定一个微信账号。前端显式发起有界 poll/send，响应只投影稳定 message ID、peer、文本和 cursor，不返回认证头或原始上游 payload。后台自动 poll、自动 Session/Run 和自动回复在建立持久 cursor、消息 ID、peer 到 Session 幂等账本前保持关闭。

插件扩展是 manifest-only Rust catalog。`HERMES_HOME/.synthchat/plugins` 下的直接子目录可通过有界 `plugin.json` 登记，catalog 使用独立 ETag 完成启停和移除 metadata；移除登记不删除源目录。`enabled` 不加载 entry point、不读取声明的环境变量、不向 Run 注入工具，也不恢复 Python、Node 或旧 Agent 插件执行 runtime。
...
flowchart LR
  ...
  BE --> DB["SQLite + FTS5"]
  BE --> PRODUCT["Persona + Worldbook + Moments<br/>Rust product catalog"]
  BE --> WECHAT["Rust WeChat adapter"]
  WECHAT --> ILINK["iLink HTTPS API"]
  BE --> PLUGIN["Manifest-only plugin catalog"]
...
backend/                   独立 Cargo workspace，纯 Rust Hermes-compatible backend
  ...
  src/product_catalog.rs   Persona、Worldbook、Moments SQLite 产品目录
  src/wechat.rs            iLink 配置、扫码、Persona 绑定与显式消息 adapter
  src/plugins.rs           manifest-only 本地插件目录
...
2026-07-21 当前状态补充：Rust-owned Session schema 已升至 v13，以持久 `persona_id` 固定 Session/Run 的 Persona 归属。Persona、Worldbook 与 Moments 已迁入独立 Rust SQLite 产品目录并接入 Desktop UI；Persona 与启用且绑定的 Worldbook section 会在首个模型 turn 冻结注入。微信配置、扫码登录、OS keychain 凭据、Persona 唯一绑定和显式有限 poll/send 已由 Rust adapter 提供；自动消息到 Run 仍关闭。Plugins 已收敛为 manifest-only Rust catalog 与 Desktop 管理页，不执行插件代码，也不恢复旧 Agent runtime。
...
- 保持已批准的非 Hermes 产品边界：产品目录显式写入、微信仅显式消息、插件仅 manifest metadata，见 `docs/code-impact.md` 与 `docs/api-contract.md`；
